define(['knockout'], function (ko) {
  function getCriteriaComponent(data) {
    if (data.hasOwnProperty('ConditionOccurrence'))
    ...
    else if (data.hasOwnProperty('VisitDetail')) return 'visit-detail-criteria';
    else if (data.hasOwnProperty('Observation')) return 'observation-criteria';
    else if (data.hasOwnProperty('DeviceExposure'))
      return 'device-exposure-criteria';
    else if (data.hasOwnProperty('Measurement')) return 'measurement-criteria';
      ...
      return 'image-occurrence-criteria';
  ...
  function getConceptSetName(conceptSetId, conceptSetList, defaultName) {
    var selectedConceptSet = conceptSetList().find(function (item) {
      return item.id == ko.utils.unwrapObservable(conceptSetId);
    });
    return (
