define([
  ...
  function CriteriaGroupViewModel(params) {
    ...
    self.groupCountOptions = ko.pureComputed(function () {
      ...
      for (
        var i = 0;
        i < self.group().CriteriaList().length + self.group().Groups().length;
        i++
      ) {
    ...
    self.addActions = [
        ...
        action: function () {
          self
            ...
            .DemographicCriteriaList.push(
              new criteriaTypes.DemographicCriteria()
            );
        },
      },
      {
        ...
        action: function () {
          ...
          self.group().CriteriaList.push(
            new AdditionalCriteria(
              {
                Criteria: {
                  ConditionEra: {},
                },
      ...
      },
      {
        ...
        action: function () {
          ...
          self
            .group()
            .Groups.push(
              new CriteriaGroup(null, unwrappedExpression.ConceptSets)
            );
        },
      },
      {
      ...
      {
        ...constants.groupAttributes.addImageOccurrence,
        selected: false,
        action: function () {
          var unwrappedExpression = ko.utils.unwrapObservable(self.expression);
          self.group().CriteriaList.push(
    ...
    self.getDistinctOptions = function (criteria) {
      let distinctOptions = [
        { id: 'DOMAIN_CONCEPT', name: 'Standard Concept' },
    ...
    self.insertFromReusable = (expression, conceptSets) => {
      if (conceptSets.length > 0) {
        ...
        unwrappedExpression.ConceptSets(
          unwrappedExpression.ConceptSets().concat(conceptSets)
        );
      }
